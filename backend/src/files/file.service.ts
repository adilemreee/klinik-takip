...
export class FileService {
  ...
  /**
   * Stores bytes this server produced itself.
   *
   * The difference from `upload` is where the type comes from. An uploaded file
   * carries a claim by an untrusted client, so its type is sniffed and anything
   * unrecognised is refused. A generated export carries no claim — this process
   * made the bytes — and the formats it makes (CSV has no magic number at all,
   * XLSX is indistinguishable from any other zip) cannot be sniffed without
   * either lying or widening the signature list that protects real uploads.
   *
   * So this method exists, and it is only ever called with content the server
   * built. Nothing that came from a user goes through it.
   */
  async store(
    bucket: FileBucket,
    source: Readable,
    options: { mime: string; extension: string; maxBytes: number },
  ): Promise<StoredFile> {
    ...
    const { size, checksum } = await this.putWithLimit(
      ...
      key,
      source,
      { mime: options.mime, extension: options.extension },
      options.maxBytes,
    );

    this.logger.log(`Stored ${size} generated bytes as ${bucket}/${key} (${options.mime})`);

    return { key, mime: options.mime, size, checksum };
  }
