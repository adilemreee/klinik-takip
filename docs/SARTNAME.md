...
- [x] T6.4 Analitik dashboard (ameliyat sayısı, ülke dağılımı, doluluk, gelir) ([ANALITIK](ANALITIK.md)) — **paydasız oran yok**: az vakadan oran üretilmiyor (`null`, sıfır değil) ve iki istemcide de sayıya çevrilemiyor; kanal raporunda gelir yetkisi olmayana **gizlendiği söyleniyor** (`revenueWithheld`), çünkü boş sütun "hiç kazanmadı" diye okunur; dönüşümün tanımı yanıtta taşınıyor; serbest metin kanallar katlanıyor ve **kaynağı yazılmamışlar kendi satırında**; gelir–gider faturanın kesildiği günün kuruyla, iptaller dışarıda ve **sayılı**; okunamayan maliyet satırı **sayılıyor**; doluluk kliniğin duvar saatiyle ve **çalışma saati tanımsızsa oran yok**; aylar klinik takviminde ve boş aylar listede. **Kapasite tahmini bilerek yapılmadı** — eğilim çıkaracak veri yok
- [x] T6.5 PDF hasta özet raporu (şablonlu) ([HASTA-OZET-PDF](HASTA-OZET-PDF.md)) — **hiçbir şey sessizce çıkarılmıyor**: doğrulanmamış lab, onaylanmamış AI metni ve onamsız fotoğraf rapora girmiyor ama **sayısı ve nedeni** hem belgede hem manifestte hem denetimde yazıyor; fotoğraflar **varsayılan kapalı** ve yalnız canlı onamla; Türkçe için Unicode TTF gömülü (alfabenin tamamı testte, yazı tipi yoksa **hiç çizilmiyor**); grafikler vektör ve tek ölçüm/düz seri/aynı an gibi bozuk durumlar test edilmiş; export **kuyrukta**, bildirimle, **kısa ömürlü imzalı link**, hem bitişte hem indirmede **denetim kaydı**; dosya 7 gün sonra siliniyor ama **kaydı kalıyor** ve istemciler "süresi doldu" ile "başarısız"ı ayırıyor. **Klinik logosu klinikten bekleniyor**
- [x] T6.6 Excel/CSV toplu export + kuyruk + indirme linki ([HASTA-OZET-PDF](HASTA-OZET-PDF.md#toplu-excelcsv-export-t66)) — **elektronik tablo formülü enjeksiyonu** her hücrede etkisizleştiriliyor (hasta kendi adını kendi yazıyor; CSV'de de XLSX'te de); kolonlar kapalı katalogdan ve yetkisiz kolon **istendiği anda reddediliyor, sessizce düşürülmüyor** — worker'da yetkiler **yeniden** çözülüyor; dosyanın içinde künye (kim/ne zaman/filtre/satır sayısı/**kapsam uyarısı**) ve kesildiyse kesildiği yazıyor; CSV'de **BOM** (yoksa Excel Türkçeyi bozar); satırlar sayfa sayfa akıtılıyor, çok para birimli tutar **boş bırakılıp `KARIŞIK`** deniyor. **Ayrı finansal rapor şablonu yapılmadı**
- [x] T6.7 PROM anketleri ([PROM-ANKETLERI](PROM-ANKETLERI.md)) — **hiçbir lisanslı ölçek depoya konmadı** (SF-36/FACE-Q telifli; değiştirilmiş bir ölçek yayınlanmış normlarla karşılaştırılamaz) ve **NPS markası kullanılmadı**; yön **soru başına yazılı** (ters okumak her uyarıyı tersine çevirir, testi var); eğilim = hastanın **kendi** önceki cevabına göre 3+ puan **ya da** eşik aşımı — dalgalanmada uyarı veren sistem kapatılan sistemdir; **ilk ankette eğilim yok**, eğri uydurulmuyor; uyarı **atanmış ekibe**, hastaya klinik yorum **dönmüyor**; uymayan cevap **reddediliyor** (boşluk sıfıra çevrilmiyor), yarım form **kısmi** işaretleniyor; geç cevap kabul edilmiyor; sürüm dondurulmuş; tarih ertelenirse cevaplanmamışlar taşınıyor, **cevaplanmışlar korunuyor**. **Başlangıç anketi ve eşikleri klinik gözden geçirmesi bekliyor**

### FAZ 7 — Sertleştirme ve Yayın
- [x] T7.1 Yük testi (k6, 500 eşzamanlı) ve darboğaz optimizasyonu ([YUK-TESTI](YUK-TESTI.md)) — 500 eşzamanlı hastada **%0 hata, p95 36 ms**; 50 eşzamanlı personel aramasında %0 hata, p95 106 ms. Yerel donanımda koşturuldu, staging sunucusuna **yöneltilmedi** (21 başka servis barındırıyor). Test üç şey buldu: hız sınırı koda gömülüydü (artık ortam değişkeni, varsayılan aynı), **arama Türkçe harfleri katlamıyordu** (`yil` → `Yılmaz` bulunmuyordu; düzeltildi), ve koordinatör kapsamı doğru çalışıyordu
...
- [~] T7.4 Erişilebilirlik denetimi (VoiceOver/TalkBack) ([ERISILEBILIRLIK](ERISILEBILIRLIK.md)) — **otomatik kısım bitti**: dört mekanik kural için denetleyici yazıldı, CI'ya bağlandı, bulduğu **17 bulgunun hepsi düzeltildi** (dokuz dekoratif ikon, beş bekleme göstergesi, üç dokunma hedefi). **Kalan kısım bir betiğin yapamayacağı**: okuma sırası, etiket ifadesi, odak davranışı ve dinamik yazı tipi — gerçek cihazda VoiceOver/TalkBack ile, tercihen ekran okuyucuyu günlük kullanan biriyle. Gerçek hasta verisinden önce
